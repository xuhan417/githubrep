...
# utilized a different agent for overtaking, set the directory accordingly
# CARLA default directory: agents.navigation.behavior_agent_overtaking
from path.to.overtaking_agent import BehaviorAgent
...
# ==============================================================================
# -- Cost Function at intersection ---------------------------------------------
# ==============================================================================
from turn_on_red_cost_fn import turn_on_red_cost
...
class World(object):
    ...
    def restart(self, args):
        ...
        while self.player is None:
            ...
            spawn_point = carla.Transform(default_spawn_points[268].location)
            # spawn vehicle
            self.player = self.world.try_spawn_actor(blueprint, spawn_point) 
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        self._info_text += [
            ...
            'Overtaking Cost: % 10f' %turn_on_red_cost_score,
...
def game_loop(args):
    ...
    try:
        ...
        # set initial target 
        destination = spawn_points[262].location
        ...
        if_stay_at_overtaking_lane = False
        agent.set_overtaking_behavior(if_stay_at_overtaking_lane)
        # read initial values
        ...
        # sapwn vehicles 
        start_loc = spawn_points[268].location
        start_rotation = spawn_points[268].rotation
        # v2
        spwan_location = carla.Location(start_loc.x+52, \
                                        start_loc.y,\
                                        start_loc.z)
        spawn_point_2 = carla.Transform(spwan_location, start_rotation) 
        ...
        start_loc_3 = spawn_points[258].location
        start_rotation_3 = spawn_points[258].rotation
        spwan_location_3 = carla.Location(start_loc_3.x-1.0-3.0, \
                                        start_loc_3.y-65,\
                                        start_loc_3.z)
        spawn_point_3 = carla.Transform(spwan_location_3, start_rotation_3) 
        ...
        while True:
            ...
            if len(agent.get_local_planner().waypoints_queue) <= 9 and args.loop:
                ...
                if len(agent._local_planner.waypoints_queue) == 0:
                ...
                if agent.tot_target_reached == 1 or agent.tot_target_reached%4 == 1: 
                    # reach 1st goal 
                    if agent.is_stay_lfet: 
                        destination = spawn_points[257].location #258
                        destination.y -= 85
                        print("New destination: " + str(destination))
                        agent.set_destination(new_start, destination)
                    else: 
                        # reach first goal 
                        destination = spawn_points[258].location #259
                        destination.y -= 85
                        print("New destination: " + str(destination))
                        agent.set_destination(new_start, destination)
                    ...
                    # reach 2nd goal 
                    if agent.is_stay_lfet: 
                        destination = spawn_points[169].location #189
                        destination.y -= 90
                        print("New destination: " + str(destination))
                        agent.set_destination(new_start, destination)
                    else: 
                        destination = spawn_points[179].location #189
                        destination.y -= 90
                        print("New destination: " + str(destination))
                        agent.set_destination(new_start, destination)
                    ...
                    # reach 3rd goal 
                    if agent.is_stay_lfet:
                        destination = spawn_points[169].location
                        print("New destination: " + str(destination))
                        agent.set_destination(new_start, destination)
                    else: 
                        destination = spawn_points[179].location
                        print("New destination: " + str(destination))
                        agent.set_destination(new_start, destination)
                    ...
                    # reach 4th goal 
                    if agent.is_stay_lfet:
                        destination = spawn_points[49].location
                        print("New destination: " + str(destination))
                        agent.set_destination(new_start, destination)
                    else:
                        destination = spawn_points[50].location
                        print("New destination: " + str(destination))
                        agent.set_destination(new_start, destination)
